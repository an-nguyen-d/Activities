...
final class ActivityGeneralTabView: BaseView {
  ...
  // Delete Section
  private let deleteSectionSeparator = updateObject(UIView()) {
    $0.backgroundColor = .separator
  }
  
  private let deleteActivityLabel = updateObject(UILabel()) {
    $0.text = "DANGER ZONE"
    $0.font = .systemFont(ofSize: 12, weight: .regular)
    $0.textColor = .systemRed
  }
  
  let deleteActivityButton = updateObject(BaseButton()) {
    $0.setTitle("Delete Activity", for: .normal)
    $0.titleLabel?.font = .systemFont(ofSize: 16, weight: .regular)
  ...
  override func setupSubviews() {
    ...
    // Add some spacing before delete section
    let spacer = UIView()
    spacer.heightAnchor.constraint(equalToConstant: 40).isActive = true
    stackView.addArrangedSubview(spacer)
    
    // Add delete section
    stackView.addArrangedSubview(deleteSectionSeparator)
    deleteSectionSeparator.heightAnchor.constraint(equalToConstant: 1).isActive = true
    
    stackView.addArrangedSubview(deleteActivityLabel)
    stackView.addArrangedSubview(deleteActivityButton)

    scrollView.fillView(self)
    contentView.fillView(scrollView)

    let stackInsets = UIEdgeInsets(top: 20, left: 20, bottom: 40, right: 20)
    stackView.anchor(
